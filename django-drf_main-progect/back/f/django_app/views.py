...
class CommentsWebSite(View):
    def get(self, request : HttpRequest):
        comments = models.Commentses.objects.all()
        return render(
            request=request,
    ...
    def post(self, request : HttpRequest):
        ...
        if text:
            models.Commentses.objects.create(text=text)
            return redirect(reverse("Feedback"))
        return render(
            request=request,
            template_name="Feedback.html",
            context={"comments": models.Commentses.objects.all(), "error": "Text cannot be empty"}
        )
